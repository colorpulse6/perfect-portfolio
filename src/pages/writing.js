...
import SEO from "../components/seo"
 ...
 const Writing = ({ transitionStatus, location }) => {
   ...
   const story = (title) =>{
return (
     data.allWriting.nodes.map((story, index)=>{if(story.title === title){
       return (<div className="story-container"><p className="rendered-title">{story.title}</p><p key={index} dangerouslySetInnerHTML={{ __html: story.content }}/></div>)
     }}))
   }
      ...
      <div  style={{ opacity: 0, position: "relative"}} className="writings">
        ...
        <div className="title-container" id="examples">
          {data.allWriting.nodes.map((story, index)=>{
            return (<div key={index} className={`${active === index ? "story-card " : "story-card"}`} onClick={()=>{
              setRenderStory(story.title);
              setActive(index)}}>
            <Spring>
                <div className="example"><span className={`${active === index ? "story-item selected background-video" : "hover hover-1 story-item"}`}>{story.title}</span>
                </div>
            </Spring>
            </div>)
          })}
